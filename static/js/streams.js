...
const APP_ID = '149eb914dd5644e48ed8a2173772564b'
...
let handleUserJoined = async (user, mediaType) => {
    ...
    if (mediaType === 'video') {
        ...
        let mem = await getMember(user)
        

        player = `<div class="video-container" id="user-container-${user.uid}">
        <div class="username-wrapper"><span id="user-name">${mem.name}</span></div>
...
let toggleCamera = async(e)=>{
    if(localTracks[1].muted){
        ...
        e.target.style.backgroundColor = 'rgb(39, 225, 193)'
    }
}
